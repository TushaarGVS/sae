...
class Griffin(nn.Module):
    ...
    def forward(
        ...
        xai_intermediate_layer_nums: List[int] | None = None,
        ...
        Args:
          ...
          xai_intermediate_layer_nums: Layer numbers to extract the explainability intermediates from.
        ...
        if not return_logits and not return_cache:
            return None, None
        if xai_intermediate_layer_nums is None:
            xai_intermediate_layer_nums = []
        ...
        for i, block in enumerate(self.blocks):
            ...
            if self.gradient_checkpointing:
                x, new_cache[block_name] = checkpoint.checkpoint(
                    ...
                    return_xai_intermediates=(i in xai_intermediate_layer_nums),
                    use_reentrant=False,
                    determinism_check="none",
                )
            else:
                x, new_cache[block_name] = block(
                    x,
                    segment_pos,
                    block_cache,
                    return_cache=return_cache,
