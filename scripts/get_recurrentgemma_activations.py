# CUDA_VISIBLE_DEVICES=0,1,3 torchrun --standalone --nproc_per_node=3 scripts/get_recurrentgemma_activations.py \
...
def _unpad_batch(
    ...
    pad_lengths: List[int] | torch.Tensor,
    debug: bool = False,
) -> List[torch.Tensor]:
...
def get_activations(
    ...
    debug: bool = False,
) -> Dict[str, Dict[str, List[torch.Tensor] | None]]:
    weights_dir = Path(
    ...
    padded_tokens = sampler._get_padded_tokens(all_input_ids)
    ...
    for layer_num in layer_nums:
        ...
        activations_dict[block_id] = dict(
            ...
            rg_lru_states=(
                _unpad_batch(block_cache.all_rg_lru_states, pad_lengths)
                if hasattr(block_cache, "all_rg_lru_states")
                else None
            ),
            mlp_activations=_unpad_batch(block_cache.mlp_activations, pad_lengths),
        )
...
def main(
    ...
    for batch_idx, batch in enumerate(tqdm(dataloader)):
        if debug:
            print(f"[DEBUG]\t {local_rank=} {world_size=}")
        ...
        with open(
            os.path.join(save_dir, f"artefacts/batch.{batch_idx}.pkl"), "wb"
        ) as fp:
