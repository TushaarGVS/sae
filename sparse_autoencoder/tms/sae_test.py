...
toy_model.bias = torch.tensor([-0.2757, -0.2749, -0.2732, -0.2761, -0.2733])
toy_model.eval()
toy_model.cuda()
...
def test_fast_tms_sae(n_steps: int = 15):
    fast_tms_sae.pre_bias = tms_sae.pre_bias
    fast_tms_sae.latent_bias = tms_sae.latent_bias
    fast_tms_sae.W_enc = tms_sae.W_enc
    fast_tms_sae.W_dec = tms_sae.W_dec
    fast_tms_sae.stats_last_nonzero = tms_sae.stats_last_nonzero

    for step in range(n_steps):
        with torch.inference_mode():
            x = generate_feature_batch(
                batch_size=10, n_features=5, feature_probs=feature_proba
            )
        ...
        loss_sae = mse_loss(activations, recons)
        loss_fast_sae = mse_loss(activations, fast_recons)
        loss_diff = torch.abs(loss_sae - loss_fast_sae).item()
        print(f"\t+ {loss_diff=}")

        loss_sae.backward()
        loss_fast_sae.backward()
