...
class ShardingComms:
    ...
    def dp_allreduce_(self, autoencoder) -> None:
        ...
        for param in autoencoder.parameters():
            if param.grad is not None:
                self.dp_comm.all_reduce(maybe_transpose(param.grad), op=ReduceOp.AVG,
                                        async_op=True)
    ...
    def sh_allreduce_scale(self, scaler):
        ...
        if hasattr(scaler, "_scale") and scaler._scale is not None:
            ...
            self.sh_comm.all_reduce(scaler._growth_tracker, op=ReduceOp.MIN,
                                    async_op=True)
...
def make_torch_comms(n_op_shards=4, n_replicas=2):
    ...
    shard_rank_lists = [list(range(i, i + n_op_shards)) for i in
                        range(0, world_size, n_op_shards)]

    shard_groups = [dist.new_group(shard_rank_list) for shard_rank_list in
                    shard_rank_lists]

    my_shard_group = shard_groups[my_replica_idx]

    replica_rank_lists = [
        list(range(i, n_op_shards * n_replicas, n_op_shards)) for i in
        range(n_op_shards)
    ]

    replica_groups = [dist.new_group(replica_rank_list) for replica_rank_list in
                      replica_rank_lists]
...
def sharded_topk(x, k, sh_comm, capacity_factor=None):
    ...
    all_vals = torch.empty(sh_comm.size(), batch, k_in, dtype=vals.dtype,
                           device=vals.device)
    ...
    def forward(self, x):
        class EncWrapper(torch.autograd.Function):
            ...
            def backward(ctx, _, grad_vals, __, grad_auxk_vals):
                ...
                grad_sum = torch.zeros(self.n_dirs_local, dtype=torch.float32,
                ...
                return (
                    ...
                    -(grad_sum @ weight),
                    triton_sparse_transpose_dense_matmul(all_inds, all_grad_vals, x,
                                                         N=self.n_dirs_local),
                    grad_sum,
                )
...
def unit_norm_decoder_grad_adjustment_(autoencoder) -> None:
    ...
    triton_add_mul_(
        ...
        torch.einsum("bn,bn->n", autoencoder.decoder.weight.data,
                     autoencoder.decoder.weight.grad),
...
def training_loop_(
        ae, train_acts_iter, loss_fn, lr, comms, eps=6.25e-10, clip_grad=None,
        ema_multiplier=0.999, logger=None
):
    if logger is None:
    ...
    for i, flat_acts_train_batch in enumerate(train_acts_iter):
        ...
        if clip_grad is not None:
            ...
            torch._foreach_mul_(grads,
                                clip_grad / torch.clamp(grad_norm, min=clip_grad))
...
def init_from_data_(ae, stats_acts_sample, comms):
    ...
    ae.pre_bias.data = (
        compute_geometric_median(
            stats_acts_sample[:32768].float().cpu()).median.cuda().float()
    )
...
class EmaModel:
    def __init__(self, model, ema_multiplier):
        ...
        self.ema_weights = [torch.zeros_like(x, requires_grad=False) for x in
                            model.parameters()]
        self.ema_steps = 0
    ...
    def use_ema_weights(self):
        ...
        ema_weights_bias_corrected = torch._foreach_div(self.ema_weights,
                                                        bias_correction)
...
def main():
    ...
    mse_scale = (
            1 / ((stats_acts_sample.float().mean(
        dim=0) - stats_acts_sample.float()) ** 2).mean()
    )
    ...
    training_loop_(
        ...
        lambda ae, flat_acts_train_batch, recons, info, logger: (
            # MSE
                logger.logkv("train_recons",
                             mse_scale * mse(recons, flat_acts_train_batch))
                # AuxK
                + logger.logkv(
            "train_maxk_recons",
            cfg.auxk_coef
