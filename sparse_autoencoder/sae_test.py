...
from sparse_autoencoder.tms.sae import TmsAutoencoder
...
fast_sae = FastAutoencoder(n_features=5, d_model=2, k=1, dead_steps_threshold=5, auxk=3)
fast_sae.cuda()
...
def test_fast_sae(n_steps: int = 15):
    fast_sae.pre_bias.data = tms_sae.pre_bias.data
    fast_sae.latent_bias.data = tms_sae.latent_bias.data
    fast_sae.W_enc.data = tms_sae.W_enc.data
    fast_sae.W_dec.data = tms_sae.W_dec.data
    fast_sae.stats_last_nonzero.data = tms_sae.stats_last_nonzero.data
    ...
    for step in range(n_steps):
        ...
        fast_recons, fast_auxk_dict = fast_sae(activations)
        ...
        stats_last_nonzero_diff = _max_diff(
            tms_sae.stats_last_nonzero, fast_sae.stats_last_nonzero
        )
        print(
            f"[{step}/{n_steps - 1}]\n\t"
            f"+ {recons_diff=}\n\t"
        ...
        dpre_bias_diff = _max_diff(tms_sae.pre_bias.grad, fast_sae.pre_bias.grad)
        dlatent_bias_diff = _max_diff(
            tms_sae.latent_bias.grad, fast_sae.latent_bias.grad
        )
        dW_enc_diff = _max_diff(tms_sae.W_enc.grad, fast_sae.W_enc.grad)
        dW_dec_diff = _max_diff(tms_sae.W_dec.grad, fast_sae.W_dec.grad)
        print(
            f"\t+ {dpre_bias_diff=}\n\t"
            f"+ {dlatent_bias_diff=}\n\t"
            f"+ {dW_enc_diff=}\n\t"
        ...
        fast_sae.unit_norm_decoder_()
        fast_sae.unit_norm_decoder_grad_adjustment_()
